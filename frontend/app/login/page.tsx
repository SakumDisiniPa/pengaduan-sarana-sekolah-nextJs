...
export default function LoginPage() {
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    setError(null);
    try {
      const authRes = await pb.collection("_pb_users_auth_").authWithPassword(email, password);
      // redirect admin to dashboard, others to chat
          ...
          <form className="mt-8 space-y-6" onSubmit={submit}>
            ...
            <div>
              <button
                type="submit"
                disabled={isLoading}
                className="group relative flex w-full justify-center rounded-lg bg-gradient-to-r from-blue-600 to-purple-600 px-4 py-3 text-sm font-semibold text-white shadow-lg transition-all duration-300 hover:scale-105 hover:shadow-xl active:scale-95 disabled:cursor-not-allowed disabled:opacity-70"
              >
